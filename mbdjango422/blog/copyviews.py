...
def post_list(request, tag_slug=None, category_id=None, category_name=None):
    if not category_id and not tag_slug:
        object_list = cache.get("all_published_posts")
        if not object_list:
            object_list = Post.published.all()
            cache.set("all_published_posts", object_list, 60 * 15)
    ...
    if tag_slug:
        ...
        object_list = cache.get('post_slug ' + str(tag_slug))
        if not object_list:
            # object_list = object_list.filter(tags__in=[tag])
            object_list = cache.get("all_published_posts").filter(tags__in=[tag])
            cache.set('post_slug ' + str(tag_slug), object_list, 60 * 15)
        canonical = False

    if category_id:
        category = get_object_or_404(Category, id=category_id)
        # todos los posts de la categoria utilizando el related_name
        object_list = cache.get('posts_category ' + str(category_id))
        if not object_list:
            object_list = category.get_posts.all()
            cache.set('posts_category ' + str(category_id), object_list, 60 * 15)
        canonical = False
